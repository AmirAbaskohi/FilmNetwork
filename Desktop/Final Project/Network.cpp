...
void Network :: handle_post_commands(vector < string > words)
{
  ...
  else if(words[1] ==  "money")
  {
    if(words.size() == 2)
    {
      if(pactive_user == NULL || is_active_publisher == false || is_admin == true)
        throw PermissionEx();
    ...
    else if(words.size() == 5)
    {
      if(words[3] != "amount")
        throw NotFoundEx();
      if(cactive_user == NULL || is_admin == true)
        throw PermissionEx();
  ...
  else if(words[1] == "buy")
  {
    ...
    {
      if(pactive_user == NULL || is_admin == true)
        throw PermissionEx();
...
void Network :: handle_get_commands(vector < string > words)
{
  ...
  else if(words[1] == "money")
  {
    ...
    if(is_admin == true)
    {
      cout << get_all_money_in_network() << endl;
      return;
    }
    cout << cactive_user -> get_all_money() << endl;
  }
  else
    throw NotFoundEx();
}
