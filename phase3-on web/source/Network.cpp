...
vector< string > Network :: show_dateils_of_movie(int film_id)
{
  ...
  for(int i = 0 ; i < films.size() ; i++)
    if(films[i]->get_id() == film_id)
    {
      ...
      details_and_recomms.push_back(to_string(films[i]->get_length()));
      details_and_recomms.push_back(films[i]->get_director());
      details_and_recomms.push_back(to_string(films[i]->get_price()));
      details_and_recomms.push_back(to_string(films[i]->get_mean_rate()));
      details_and_recomms.push_back(to_string(films[i]->get_year()));
...
vector<vector < string > > Network :: get_purchased(string director)
{
  vector<vector < string > >all_films_inf;
  vector<string>one_film_inf;
  for(int i = 0 ; i < cactive_user -> get_size_films() ; i++)
  ...
  for(int i = 0 ; i < films.size() ; i++)
  {
    if(cactive_user->get_all_money() >= films[i]->get_price())
      if(films[i]->get_director() == director || director=="")
      {
        one_film_inf.push_back(films[i] -> get_name());
        one_film_inf.push_back(to_string(films[i] -> get_length()));
        one_film_inf.push_back(to_string(films[i] -> get_price()));
        one_film_inf.push_back(to_string(films[i] -> get_mean_rate()));
        one_film_inf.push_back(to_string(films[i] -> get_year()));
        one_film_inf.push_back(films[i] -> get_director());
        all_films_inf.push_back(one_film_inf);
        one_film_inf.clear();
      }
...
int Network :: get_all_money_of_current_user()
{
  return cactive_user->get_all_money();
}
void Network :: change_all_money_of_current_user(int amount)
{
