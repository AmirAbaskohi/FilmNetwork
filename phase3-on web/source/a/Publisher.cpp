...
vector< vector<string> > Publisher :: get_published(string director)
{
	vector<string>one_film_inf;
	vector< vector<string> >all_films_information;
	for(int i = 0 ; i < published_films.size() ; i++)
		if(published_films[i]->get_director() == director || director == "")
		{
			one_film_inf.push_back(published_films[i]->get_name());
			one_film_inf.push_back(to_string(published_films[i]->get_length()));
			one_film_inf.push_back(to_string(published_films[i]->get_price()));
			one_film_inf.push_back(to_string(published_films[i]->get_mean_rate()));
			one_film_inf.push_back(to_string(published_films[i]->get_year()));
			one_film_inf.push_back(published_films[i]->get_director());
			one_film_inf.push_back(to_string(published_films[i]->get_id()));
			all_films_information.push_back(one_film_inf);
			one_film_inf.clear();
		}
	return all_films_information;
}
